...
date_sufix = ['lintang', 'mangkin', 'iriki','kaon', 'riin', 'liwat', 'inuni', 'rauh', 'anu']
    ...
    for index, i in enumerate(txtPrep):
        ...
        if i in day and txtPrep[index-1] not in date_prefix:
            ...
            if txtPrep[index+2] in month:
                if txtPrep[index+3].isnumeric()==True:
                    a = txtPrep[index+1] + ' ' + txtPrep[index+2] + ' ' + txtPrep[index+3]
                else:
                    a = txtPrep[index+1] + ' ' + txtPrep[index+2]
                # a = i + ' ' + txtPrep[index+1] + ' ' + txtPrep[index+2] + ' ' + txtPrep[index+3]
                pWaktu.append(a)
            if txtPrep[index-1] not in date_prefix and txtPrep[index+2] not in month:
                pWaktu.append(i)
        if i in month and txtPrep[index-1] not in date_prefix:
            if txtPrep[index-1].isnumeric()==False and txtPrep[index+1].isnumeric()==True and len(txtPrep[index+1]) == 4:
                a = i + txtPrep[index+1]
                pWaktu.append(a)
    return pWaktu
...
def ner_time(sentences):
    ...
    # print(pWaktu)
    return list(dict.fromkeys(pWaktu))
